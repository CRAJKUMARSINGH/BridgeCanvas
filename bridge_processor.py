...
class BridgeProcessor:
    ...
    def process_excel_file(self, filepath, project_name=None):
        ...
        try:
            ...
            return {"success": False, "error": str(e), "variables": {}, "dxf_filename": None, "svg_content": None}
    ...
    def draw_bridge_elevation(self, msp, variables, sc, skew1):
        ...
        try:
            ...
            bridge_points = [(left, sofl), (left + lbridge, sofl), (left + lbridge, toprl), (left, toprl), (left, sofl)]
    ...
    def draw_bridge_superstructure(self, msp, variables, hpos, vpos, scale1, hhs):
        ...
        try:
            ...
            for i in range(1, nspan):
                ...
                msp.add_lwpolyline(
                    [new_pta1, [new_pta2[0], new_pta1[1]], new_pta2, [new_pta1[0], new_pta2[1]], new_pta1], close=True
                )
    ...
    def draw_left_abutment(self, msp, variables, hpos, vpos, scale1):
        ...
        try:
            ...
            msp.add_lwpolyline(
                [pt1, pt2, pt3, pt4, pt5, pt6, pt7, pt8, pt9, pt10, pt11, pt12, pt13, pt14, pt1], close=True
            )
    ...
    def draw_complex_pier_geometry(self, msp, variables, hpos, vpos, scale1, hhs):
        ...
        try:
            ...
            self.add_pier_dimensions(msp, pier_x, capt, capb, capw, piertw, battr, pierst, futrl, futd, futw, scale1)
    ...
    def draw_detailed_abutment_geometry(self, msp, variables, hpos, vpos, scale1):
        ...
        try:
            ...
            # Add dimension annotations
            self.add_abutment_dimensions(
                msp, pt1, pt2, pt3, pt4, pt5, pt6, pt7, pt8, pt9, pt10, pt11, pt12, pt13, pt14, pt15, scale1
            )
    ...
    def add_plan_view_dimensions(
        ...
        try:
            ...
            msp.add_text("Footing", dxfattribs={"height": 2 * scale1, "insert": (pt16[0] + 1, pt16[1] + 1)})

            # Add abutment outline dimensions
            msp.add_text("Abutment", dxfattribs={"height": 2 * scale1, "insert": (pt20[0] + 1, pt20[1] + 1)})

            # Add skew adjustment indicators
            msp.add_text("Skew Adj", dxfattribs={"height": 1.5 * scale1, "insert": (pt21[0] + 1, pt21[1] + 1)})
    ...
    def draw_cross_section_plotting(self, msp, variables, section_x, section_y, scale1):
        ...
        try:
                ...
                if excel_file and os.path.exists(excel_file):
                    ...
                    if "Chainage (x)" in df_sheet2.columns and "RL (y)" in df_sheet2.columns:
                        ...
                        for x, y in zip(chainages, rls):
                            ...
                            rounded_x = round(x, 2)
                            msp.add_text(
                                str(rounded_x), dxfattribs={"height": 2 * scale1, "insert": pta1, "rotation": 90}
                            )

                            rounded_y = round(y, 2)
                            msp.add_text(
                                str(rounded_y), dxfattribs={"height": 2 * scale1, "insert": pta2, "rotation": 90}
                            )
                        ...
                        b1 = "CH"
                        msp.add_text(
                            b2, dxfattribs={"height": 3 * scale1, "insert": (section_x - ccbr / 2 - 10, datum - 10)}
                        )
                        msp.add_text(
                            b1, dxfattribs={"height": 3 * scale1, "insert": (section_x + ccbr / 2 + 10, datum - 10)}
                        )
            ...
            self.add_cross_section_dimensions(msp, section_x, section_y, ccbr, kerbw, slbtht, wcth, scale1)
    ...
    def add_cross_section_chainages(self, msp, section_x, section_y, ccbr, noch, scale1):
        ...
        try:
            ...
            msp.add_text(
                f"CH {section_x:.1f}", dxfattribs={"height": 2.5 * scale1, "insert": (section_x, section_y + 3)}
            )

        except Exception as e:
            self.logger.error(f"Chainage annotation error: {str(e)}")

    def add_pier_dimensions(
        self, msp, pier_x, capt, capb, capw, piertw, battr, pierst, futrl, futd, futw, scale1, vpos=None
    ):
        ...
        try:
            ...
            # Cap dimensions
            msp.add_text(f"Cap: {capw:.2f}m", dxfattribs={"height": 1.5 * scale1, "insert": (pier_x, vpos(capt) + 2)})

            # Pier shaft dimensions
            msp.add_text(
                f"Top: {piertw:.2f}m", dxfattribs={"height": 1.5 * scale1, "insert": (pier_x - 3, vpos(capb) - 1)}
            )
            ...
            msp.add_text(
                f"Footing: {futw:.2f}m x {futd:.2f}m",
    ...
    def add_abutment_dimensions(
        self, msp, pt1, pt2, pt3, pt4, pt5, pt6, pt7, pt8, pt9, pt10, pt11, pt12, pt13, pt14, pt15, scale1
    ):
        """Add detailed dimension annotations for abutment elements"""
        try:
            # Main abutment dimensions
            msp.add_text("Abutment", dxfattribs={"height": 2 * scale1, "insert": (pt1[0] + 2, pt1[1] + 2)})

            # Cap dimensions
            msp.add_text("Cap", dxfattribs={"height": 1.5 * scale1, "insert": (pt3[0] + 1, pt3[1] + 1)})

            # Dirt wall annotation
            msp.add_text("Dirt Wall", dxfattribs={"height": 1.5 * scale1, "insert": (pt14[0] - 2, pt14[1] + 1)})

            # Return wall annotation
            msp.add_text("Return", dxfattribs={"height": 1.5 * scale1, "insert": (pt15[0] - 2, pt15[1] + 1)})
        ...
        try:
            ...
            msp.add_text(
                ...
                dxfattribs={
                    "height": 3 * scale1,
                    "style": "Arial",
                    "insert": (section_x, section_y + 20 * scale1),
                    "halign": 1,
                },
            )
            ...
            msp.add_text(
                ...
                dxfattribs={
                    "height": 2.5 * scale1,
                    "style": "Arial",
                    "insert": (section_x, section_y - 10 * scale1),
                    "halign": 1,
                },
            )
            ...
            msp.add_text(
                ...
                dxfattribs={
                    "height": 2.5 * scale1,
                    "style": "Arial",
                    "insert": (section_x - ccbr / 2 - 10 * scale1, section_y + 10 * scale1),
                    "halign": 2,
                },
            )
            ...
            msp.add_text(
                ...
                dxfattribs={
                    "height": 2.5 * scale1,
                    "style": "Arial",
                    "insert": (section_x, section_y + 5 * scale1),
                    "halign": 1,
                },
            )
        ...
        try:
            ...
            while current_x <= right and chainage_count < noch:
                ...
                msp.add_text(
                    ...
                    dxfattribs={
                        "height": 3 * scale1,
                        "style": "Arial",
                        "insert": (current_x, datum - 60 * scale1),
                        "halign": 1,
                    },
                )
            ...
            while current_y <= toprl and level_count < max_levels:
                ...
                msp.add_text(
                    ...
                    dxfattribs={
                        "height": 3 * scale1,
                        "style": "Arial",
                        "insert": (left - 60 * scale1, current_y),
                        "valign": 1,
                    },
                )
            ...
            msp.add_text(
                ...
                dxfattribs={
                    "height": 5 * scale1,
                    "style": "Arial",
                    "insert": ((left + right) / 2, datum - 80 * scale1),
                    "halign": 1,
                },
            )
            ...
            msp.add_text(
                ...
                dxfattribs={
                    "height": 5 * scale1,
                    "style": "Arial",
                    "insert": (left - 80 * scale1, (datum + toprl) / 2),
                    "valign": 1,
                    "rotation": 90,
                },
            )
